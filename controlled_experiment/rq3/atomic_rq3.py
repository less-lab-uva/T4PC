...
def main():
    ...
    model_id = None
    assert len(args.properties) >= 2, "Need to define at least 2 properties."
    # Base model case
    if len(args.properties) == 6 and not args.prop_loss and not args.mask_loss:
        model_id = f"base__split{args.split_n}"
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    ### Partition dataset
    base_path = Path(f"controlled_experiment/rq3/data/{args.model_type}/")
    props_string = "-".join(args.properties)
    ...
    vanilla_args_dict = {
        ...
        "id": f"{props_string}__{'pl' if args.prop_loss else 'nopl'}__split{args.split_n}" if model_id is None else model_id,
    ...
    model = ResnetModel()
    # Load weights
    aux_model_path = f"{props_string}__{'pl' if args.prop_loss else 'nopl'}__split{args.split_n}" if model_id is None else model_id
