...
from case_study.TCP.resnet import *
...
class Indicator(nn.Module):
	def __init__(self, m, p = 1e-5):
		super(Indicator, self).__init__()
		dt = torch.float32
		self.register_parameter(name='fc0', param = nn.Parameter(torch.ones((1, m), dtype=dt, requires_grad=True)))
		self.register_parameter(name='b0', param = nn.Parameter(torch.tensor([-1.0,-(1.0+p)], dtype=dt, requires_grad=True)))
		self.register_parameter(name='fc1', param = nn.Parameter(torch.tensor([1.0/p, -1.0/p], dtype=dt, requires_grad=True)))

	def forward(self, x):
		x = F.relu(x @ self.fc0 + self.b0)
		x = F.relu(x @ self.fc1.T)
		return x
	
class Condition1(nn.Module):
	def __init__(self):
		super(Condition1, self).__init__()
		self.register_parameter(name='block1', param = nn.Parameter(torch.tensor([[1.0,1.0], [1.0,0.0]])))
		self.register_parameter(name='block2', param = nn.Parameter(torch.tensor([[0.0,1.0],[0.0,0.0]])))

	def forward(self, x):
		y = x @ self.block1
		diag = torch.diag_embed(y)
		det = torch.linalg.det(diag)
		division = y.T * (1/det).T
		isolated = self.block2 @ division
		final = x @ isolated
		numbers = torch.diag(final)
		return numbers * 2 - 1
	
class Condition2(nn.Module):
	def __init__(self):
		super(Condition2, self).__init__()
		self.register_parameter(name='offset', param = nn.Parameter(torch.tensor([0.0, -2.0])))
		self.register_parameter(name='block1', param = nn.Parameter(torch.tensor([[1.0,1.0], [1.0,0.0]])))
		self.register_parameter(name='block2', param = nn.Parameter(torch.tensor([[0.0,1.0],[0.0,0.0]])))
...
class OutputAdapter(nn.Module):
	def __init__(self, p):
		super(OutputAdapter, self).__init__()
		# Conditions
		self.cond1 = Condition1()
		self.cond2 = Condition2()
		# Indicators
		self.ind_a = Indicator(2, p)
		self.ind_b = Indicator(2, p)
	
	def forward(self, x):
		# x.shape = [batch_size, 2] -> mu, sigma
		# Indicator takes in torch.Size([batch_size, 1])
		# Conditional takes in torch.Size([batch_size, 2])
		mu = x[:,0].unsqueeze(dim=1)
		sigma = x[:,1].unsqueeze(dim=1)
		r = (
			self.ind_a(mu) * self.ind_b(sigma) * self.cond2(x) + # a>1 and b>1 => a-1/a+b-2
			(1.0-self.ind_a(mu)) * (1.0-self.ind_b(sigma)) * self.cond1(x) + # a<=1 and b<=1 => a/a+b
			(1-self.ind_a(mu)) * self.ind_b(sigma) * -1 + # a<=.0 and b>1 => 0
			self.ind_a(mu) * (1-self.ind_b(sigma)) * 1 # a>1 and b<=1 => 1
		)
		return r
...
class TCP(nn.Module):
	...
	def __init__(self, config):
		...
		self.output_adapter = OutputAdapter(p=1.0e-2)
	...
	def forward(self, img, state, target_point):
		...
		# Use OutputAdapter to get steering [-1,1] and acceleration [-1,1]
		mu_sig_acc = torch.stack((outputs['mu_branches'][:,0], outputs['sigma_branches'][:,0]), dim=1)
		mu_sig_steer = torch.stack((outputs['mu_branches'][:,1], outputs['sigma_branches'][:,1]), dim=1)
		outputs['acceleration'] = self.output_adapter(mu_sig_acc)
		outputs['steering_angle'] = self.output_adapter(mu_sig_steer)
