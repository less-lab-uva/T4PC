...
from case_study.TCP.augment import hard as augmenter

class CARLA_Data(Dataset):

	def __init__(self, root, data_folders, img_aug = False, augment_data = False):
		self.root = root
		...
		for sub_root in data_folders:
			...
			if len(self.prop_eval) == 0:
				...
				for i in range(len(self.prop_eval)):
					...
					self.prop_data[i].extend(data['prop_data'][i])

		if augment_data:
			print(f"Size before aug: {len(self.front_img)}")
			self.augment_data()
			print(f"Size after aug: {len(self.front_img)}")
	...
	def augment_data(self,):
		...
		aug_indexes = {}
		aug_speeds = {}
		for i, prop_name in enumerate(self.prop_name):
			if prop_name[0] == "phi_ss_acc_v6_aug":
				aug_indexes["phi_ss_acc_v6"] = [i for i, e in enumerate(self.prop_eval[i]) if e != 0]
				aug_speeds["phi_ss_acc_v6"] = [i / 100 for i in range(0, 10)]
				print(f"phi_ss_acc_v6_aug # indexes: {len([i for i, e in enumerate(self.prop_eval[i]) if e != 0])}")
			if prop_name[0] == "phi_ss_stop_v6_aug":
				aug_indexes["phi_ss_stop_v6"] = [i for i, e in enumerate(self.prop_eval[i]) if e != 0]
				aug_speeds["phi_ss_stop_v6"] = [i / 10 for i in range(5,100,5)]
				print(f"phi_ss_stop_v6_aug # indexes: {len([i for i, e in enumerate(self.prop_eval[i]) if e != 0])}")
			if prop_name[0] == "phi_ss_acc_v6":
				print(f"phi_ss_acc_v6 # indexes: {len([i for i, e in enumerate(self.prop_eval[i]) if e != 0])}")
			if prop_name[0] == "phi_ss_stop_v6":
				print(f"phi_ss_stop_v6 # indexes: {len([i for i, e in enumerate(self.prop_eval[i]) if e != 0])}")
		for i, prop_name in enumerate(self.prop_name):
			speeds = []
			indexes = []
			if prop_name[0] == "phi_ss_acc_v6":
				indexes = aug_indexes["phi_ss_acc_v6"]
				speeds = aug_speeds["phi_ss_acc_v6"]
			if prop_name[0] == "phi_ss_stop_v6":
				indexes = aug_indexes["phi_ss_stop_v6"]
				speeds = aug_speeds["phi_ss_stop_v6"]
			
			# Add to the dataset
			if len(speeds) > 0 and len(indexes) > 0:
				for sp in speeds:
					for idx in indexes:
						for attr in list(self.__dict__.keys())[3:]:
							if attr == "speed":
								self.__dict__['speed'] += [sp]
							else:
								self.__dict__[attr] += [self.__dict__[attr][idx]]

		# Assert that all attr have the same length
		for attr in list(self.__dict__.keys())[3:]:
			if attr.startswith("prop_"):
				for i in range(len(self.prop_eval)):
					assert len(self.front_img) == len(self.__dict__[attr][i])
			else:
				assert len(self.front_img) == len(self.__dict__[attr])
